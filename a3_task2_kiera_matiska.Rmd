...
sw_split_name <- starwars %>% 
  separate(col = name, # Separates one column into two
...
sw_skin_sep <- starwars %>% 
  separate(col = skin_color, # Separate one column into two
           into = c("skin_1", "skin_2"),
           sep = ",", # The separating variable is a comma
           extra = "merge",
           remove = FALSE)
```
...
mpg_model_class <- mpg %>% 
  unite(col = model_class, # Combine two separate columns into one
        c(model, class),
        sep = "-")
```
...
ymd(19980922) # Make dates all the same
...
some_dates <- c(10071999, 09221998, 10172007) # Convert date column into a date class
...
gasoal_hookshots <- lakers %>% # Create a new data frame
  filter(player == "Pou Gasol" | type == "hook") %>% 
  mutate(date = ymd(date)) %>% 
  mutate(date_month = month(date)) %>% # Create a new column
  count(date_month)
...
storms_danger <- storms %>% 
  mutate(danger = case_when( # Uses if-when statements
